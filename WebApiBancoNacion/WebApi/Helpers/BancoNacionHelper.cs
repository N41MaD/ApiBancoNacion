namespace WebApiBancoNacion.Helpers
{
    ...
    public class BancoNacionHelper
    {
        ...
        {
            List<Billete> lstBilletes = _context.Billete.ToList();

            if (lstBilletes.Count == 0)
            {
                lstBilletes.Add(new Billete() {BilleteID = "12", Descripcion = "Euro"});
                lstBilletes.Add(new Billete() {BilleteID = "22", Descripcion = "Dolar U.S.A" });
                lstBilletes.Add(new Billete() { BilleteID = "23", Descripcion = "Real (*)"});

                _context.Billete.AddRange(lstBilletes);
            }
